# Final version of app.py (ready for deployment)
...
def index():
    if request.method == "POST":
        ...
        if not passport:
            return render_template("index.html", message="Passport photo is required.")
        if not passport.filename.lower().endswith((".jpg", ".jpeg", ".png")):
            return render_template("index.html", message="Only image files (.jpg, .jpeg, .png) are allowed.")

        passport_path = f"{STATIC_FOLDER}/passport_{uuid.uuid4().hex}.jpg"
        image = Image.open(passport)
        image = image.convert("RGB")
        image = image.resize((180, 220))
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={form['regno'].replace('/', '_')}_receipt.pdf",
